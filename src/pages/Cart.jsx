    ...
    >
      <section className="mx-auto w-full max-w-7xl px-4 py-8 sm:px-6 lg:px-8">
        <div className="relative overflow-hidden rounded-lg bg-[#2f2118] px-6 py-10 text-white md:px-10">
          <div className="absolute inset-y-0 end-0 hidden w-1/2 bg-[url('/images/banner.webp')] bg-contain bg-right bg-no-repeat opacity-35 md:block" />
      ...
      <section className="mx-auto grid w-full max-w-7xl gap-6 px-4 pb-14 sm:px-6 lg:grid-cols-[1fr_380px] lg:px-8">
            ...
            cartItems.map((item) => (
              ...
              >
                ...
                <div className="flex flex-col justify-center">
                  ...
                  <h2 className="mt-1 text-xl font-bold text-[#2f2118]">
                    {item.product.name}
                  </h2>
                  <p className="mt-2 text-sm text-gray-600">
                    {lang === "en" ? "Unit price" : "سعر الوحدة"}:{" "}
                  ...
                  <div className="mt-4 flex flex-wrap items-center gap-2">
                    <button
                      ...
                      aria-label={
                        lang === "en" ? "Decrease quantity" : "تقليل الكمية"
                      }
                    >
                      <Minus size={16} />
                    </button>
                    ...
                    <button
                      ...
                      className="flex h-9 w-9 items-center justify-center rounded-lg border border-[#eadfd6] text-[#2f2118] transition hover:border-orange-400 hover:text-orange-500"
                      aria-label={
                        lang === "en" ? "Increase quantity" : "زيادة الكمية"
                      }
                    >
                      <Plus size={16} />
                    </button>
                    <button
                      onClick={() =>
                        removeFromCart(item.id, item.selectedTag, true)
                      }
                      className="ms-2 flex h-9 w-9 items-center justify-center rounded-lg border border-red-100 text-red-600 transition hover:bg-red-50"
                      aria-label={
                        lang === "en" ? "Remove product" : "حذف المنتج"
                      }
                    >
                      <Trash2 size={16} />
                    </button>
        ...
        <aside className="h-fit rounded-lg border border-[#eadfd6] bg-white p-5 shadow-sm lg:sticky lg:top-28">
          ...
          {cartItems.length > 0 && (
            <button
              onClick={clearCart}
              className="mt-3 w-full rounded-lg border border-[#eadfd6] px-5 py-3 font-semibold text-[#2f2118] transition hover:border-red-200 hover:bg-red-50 hover:text-red-600"
            >
